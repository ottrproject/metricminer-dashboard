...
2. Make sure your GitHub secret has the `repo` and `workflow` scopes box checked. Name it something that will remind you it has to do with your metricminer dashboard Github data and then scroll to the bottom and click "Generate Token". Keep this page open for now.
3. Return to your metricminer dashboard repository and go to Settings > Secrets and variables > Actions.
4. Click on `New repository secret`. Name your new secret *exactly* `METRICMINER_GITHUB_PAT`. OTTR uses this secret for general website tasks, including checks and rendering.
5. Copy and paste your Personal access token in the `Secret` box and then click the green "Add secret" button.
metricminer uses this secret when collecting data from GitHub repositories.
